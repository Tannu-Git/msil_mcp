...
**Phase 3 Update**: Added Exposure Governance (Section 2a) - Two-layer security model
...
## 2a. Exposure Governance (Layer B Security) - Phase 1-3 Addition

### 2a.1 Overview

**Exposure Governance** provides a **two-layer security model** that separates tool visibility from tool execution:
...
This separation provides defense-in-depth and optimizes token usage by only showing users relevant tools.

**File References**: 
- `mcp-server/app/core/exposure/manager.py`
- `mcp-server/app/api/admin.py` (admin endpoints)
- `admin-ui/src/pages/Exposure.tsx` (admin UI)

### 2a.2 Exposure Check Flow

```
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│                          EXPOSURE GOVERNANCE FLOW                                        │
├─────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                         │
...
│                                                                                         │
│  ┌───────────────────────────────────────────────────────────────────────────────────┐  │
│  │ STEP 1: Get Exposure Permissions (Layer B)                                        │  │
│  │                                                                                   │  │
│  │ File: mcp-server/app/core/exposure/manager.py                                     │  │
│  │                                                                                   │  │
│  │ 1. Extract user roles from request headers: ["operator"]                          │  │
│  │ 2. Check in-memory cache (TTL: 3600s):                                            │  │
│  │    - Cache key: "operator" (sorted roles joined)                                  │  │
│  │    - If cached and not expired → Return cached permissions (1ms)                  │  │
│  │ 3. Cache miss → Query database:                                                   │  │
│  │    SELECT permission FROM policy_role_permissions                                  │  │
│  │    WHERE role_id = (SELECT id FROM policy_roles WHERE name = 'operator')          │  │
│  │    Result: ["expose:bundle:customer-service"]                                     │  │
│  │ 4. Parse permissions:                                                              │  │
│  │    - expose:bundle:customer-service → "__BUNDLE__customer-service"                │  │
│  │ 5. Cache result with timestamp: ({"__BUNDLE__customer-service"}, time.time())     │  │
│  └───────────────────────────────────────────────────────────────────────────────────┘  │
│                                         │                                               │
│                                         ▼                                               │
│  ┌───────────────────────────────────────────────────────────────────────────────────┐  │
│  │ STEP 2: Filter Tools by Exposure                                                  │  │
│  │                                                                                   │  │
│  │ For tools/list:                                                                    │  │
│  │   1. Get all active tools from registry (250 tools)                               │  │
│  │   2. Filter by exposure:                                                           │  │
│  │      - If expose:all → Return all tools                                            │  │
│  │      - If expose:bundle:* → Return tools where tool.bundle_name matches           │  │
│  │      - If expose:tool:* → Return tools where tool.name matches                    │  │
│  │   3. Result: 15 tools visible to operator                                          │  │
│  │                                                                                   │  │
│  │ For tools/call:                                                                    │  │
│  │   1. Check if requested tool is in exposed set:                                   │  │
│  │      - Tool: "create_booking"                                                      │  │
│  │      - Bundle: "customer-service"                                                  │  │
│  │      - Check: "__BUNDLE__customer-service" in exposed_refs → ✓ YES                │  │
│  │   2. If not exposed → Raise ToolNotAvailable error                                │  │
│  │   3. If exposed → Continue to Layer A (Authorization)                             │  │
│  └───────────────────────────────────────────────────────────────────────────────────┘  │
│                                         │                                               │
│                                         ▼ Proceed to Authorization Layer               │
│  ┌───────────────────────────────────────────────────────────────────────────────────┐  │
│  │ LAYER A: Authorization (Existing RBAC) - See Section 3                            │  │
│  └───────────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                         │
└─────────────────────────────────────────────────────────────────────────────────────────┘
```

### 2a.3 Permission Types

| Permission Format | Description | Example | Tools Visible |
|-------------------|-------------|---------|---------------|
| `expose:all` | User can see ALL tools | Admin role | 250+ tools |
| `expose:bundle:BUNDLE_NAME` | User can see all tools in specific bundle | `expose:bundle:customer-service` | 15 tools in bundle |
| `expose:tool:TOOL_NAME` | User can see specific tool only | `expose:tool:get_status` | 1 tool |

### 2a.4 Exposure Cache Performance

```python
# Cache structure: Dict[cache_key, Tuple[exposed_tools, timestamp]]
_exposure_cache = {
    "operator": (
        {"__BUNDLE__customer-service"},  # Set of tool references
        1737982800.123                   # Cached timestamp
    ),
    "analyst:operator": (
        {"__BUNDLE__customer-service", "__BUNDLE__data-analysis"},
        1737982850.456
    )
}
...
# - Subsequent requests (hot): 1ms (cache hit) → 50x faster
# - Cache TTL: 3600 seconds (1 hour, configurable)
# - Cache hit rate: >95% in production
```

### 2a.5 Exposure Validation Examples

**Example 1: Operator with Bundle Access**
```python
# User: operator@msil.com
# Roles: ["operator"]
# Database permissions: ["expose:bundle:customer-service"]

# tools/list request:
all_tools = await tool_registry.get_active_tools()  # 250 tools
exposed_tools = await exposure_manager.filter_tools(
    all_tools, "usr_op_001", ["operator"]
)
# Result: 15 tools in customer-service bundle

# tools/call request for "create_booking":
exposed_refs = await exposure_manager.get_exposed_tools_for_user(
    "usr_op_001", ["operator"]
)  # Returns: {"__BUNDLE__customer-service"}

tool = await tool_registry.get_tool("create_booking")
# tool.bundle_name = "customer-service"

is_exposed = exposure_manager.is_tool_exposed(
    "create_booking", "customer-service", exposed_refs
)
# Returns: True (bundle matches)
# → Proceed to authorization layer
```

**Example 2: Analyst with Multiple Bundles**
```python
# User: analyst@msil.com
# Roles: ["analyst", "operator"]
# Database permissions: [
#     "expose:bundle:customer-service",  # from operator
#     "expose:bundle:data-analysis"      # from analyst
# ]

# Exposure calculation (union of all role permissions):
exposed_refs = {
    "__BUNDLE__customer-service",
    "__BUNDLE__data-analysis"
}

# tools/list result: 37 tools (15 + 22 from both bundles)
```
...
# User: admin@msil.com
# Roles: ["admin"]
# Database permissions: ["expose:all"]

# Exposure calculation:
exposed_refs = {"*"}  # Special marker for all tools

# tools/list result: All 250+ tools
# tools/call: All tools pass exposure check
```
...
### 2a.7 Admin API Management

Exposure permissions are managed via 5 admin API endpoints:

```
GET    /admin/exposure/roles/{role}           # Get role permissions
POST   /admin/exposure/roles/{role}/permissions  # Add permission
...
### 2a.8 Security Benefits

| Benefit | Impact |
|---------|--------|
| **Information Disclosure Prevention** | Users can't discover internal/sensitive tools |
| **Token Efficiency** | 82% reduction in prompt tokens |
| **Defense-in-Depth** | Independent validation at tools/list and tools/call |
